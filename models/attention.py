...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None,
                attention_control_dict={}, return_dict: bool = True):
        """
        Args:
        ...
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, context=encoder_hidden_states, timestep=timestep,
                                  attention_control_dict=attention_control_dict)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states, context=None, timestep=None, attention_control_dict={}):
        # 1. Self-Attention
        norm_hidden_states = (
        ...
        if self.only_cross_attention:
            attn_out, _ = self.attn1(norm_hidden_states, context, attention_control_dict=attention_control_dict) + hidden_states
        ...
        attn_out, _ = self.attn2(norm_hidden_states, context=context, attention_control_dict=attention_control_dict)
        hidden_states = attn_out + hidden_states
...
class CrossAttention(nn.Module):
    ...
    def reshape_batch_dim_to_heads_and_average(self, tensor):
        ...
        head_size = self.heads
        tensor = tensor.reshape(batch_size // head_size, head_size, seq_len, seq_len2)
        return tensor.mean(1)

    def forward(self, hidden_states, context=None, mask=None, attention_control_dict={}):
        ...
        if self._use_memory_efficient_attention_xformers:
            ...
            if self._slice_size is None or query.shape[0] // self._slice_size == 1:
                ...
                hidden_states, attn_probs = self._attention(query, key, value)
    ...
    def _attention(self, query, key, value, **kwargs):
        ...
        attention_probs = self.reshape_batch_dim_to_heads_and_average(attention_probs)
