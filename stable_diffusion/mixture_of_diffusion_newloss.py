...
from stable_diffusion.vis_utils import caculate_loss_att_fixed_cnt, caculate_loss_self_att
...
class StableDiffusion(nn.Module):
    ...
    def update_latent(self, latent, locations, boxes,t,index1, text_emb):
        ...
        while loss.item() > loss_threshold and iteration < max_iter and (index1 < max_index):
            ...
            e_t,  att_first, att_second, att_third, self1, self2, self3 = self.unet(x, t, text_emb)
            
            bboxes = boxes
            object_positions = locations
            loss1 = caculate_loss_att_fixed_cnt(att_second,att_first,att_third, bboxes_ori=bboxes,
                                object_positions_ori=object_positions, t = index1)*loss_scale
            loss2 = caculate_loss_self_att(self1, self2, self3, bboxes=bboxes,
    ...
    def produce_latents_bg(self, text_embeddings,text_embeds_original, height=512, width=512, num_inference_steps=50, guidance_scale=7.5,
        ...
        with torch.cuda.amp.autocast():#torch.autocast('cuda'):
            for i, t in enumerate(self.scheduler.timesteps):
                ...
                noise_pred = None
                all_noise = []
                all_noise_pred_text = None
                all_nosie_pred_uncond = None
                with torch.no_grad():
                    for style_i, mask in enumerate(self.masks):
                        ...
                        if t > bg_aug_end:
                            ...
                            noise_pred_uncond, att1, att2, att3,_,_,_ = self.unet(masked_latent, t, encoder_hidden_states=text_embeddings[style_i: style_i+ 1]) 
                            noise_pred_text, att1, att2, att3,_,_,_ = self.unet(masked_latent, t, encoder_hidden_states=text_embeddings[style_i+num_mask: style_i+ 1+num_mask]) 
                    
                        else:
                            noise_pred_uncond, att1, att2, att3,_,_,_ = self.unet(masked_latent, t, encoder_hidden_states=text_embeds_original[style_i: style_i+ 1])
                            noise_pred_text, att1, att2, att3,_,_,_ = self.unet(masked_latent, t, encoder_hidden_states=text_embeds_original[style_i+num_mask: style_i+ 1+num_mask])
                        noise = noise_pred_uncond.sample + guidance_scale * ( noise_pred_text.sample - noise_pred_uncond.sample)
                        all_noise.append(noise)
                    

                        # result.append(self.scheduler.step(noise, t, latents)['prev_sample'] * mask)
                ll = self.scheduler.step(torch.cat(all_noise), t, latents)['prev_sample']     
                latents = (ll * masks ).sum(dim=0, keepdims=True)
                # noise_pred = all_nosie_pred_uncond + guidance_scale * ( all_noise_pred_text - all_nosie_pred_uncond)
                # latents = self.scheduler.step(noise_pred, t, latents)['prev_sample']
                #torch.cuda.empty_cache()
